...
raw<- cruts2raster("cru_ts4.06.1901.2021.tmp.dat.nc", timeRange=c("1901-01-01","2021-01-01"))
# Make spatial grids
...
for(i in 1: length(xy[,1])){
  ...
  Fix_out<- ColwellsP_Fix(input, breaks= tmp.breaks)  # Get the list of Colwell's predictability, constancy, and contingency
  ...
  Dym_out<- ColwellsP_Dym(input, breaks= 24)          # Get the list of Colwell's predictability, constancy, and contingency
...
fwrite(as.data.table(plot.data), "TMP_CRU1901-2020_26Oct.csv")
